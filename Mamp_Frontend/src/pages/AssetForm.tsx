...
import { assetsApi, propertiesApi } from '../api';
import { AssetRequest, AssetStatus, Property } from '../types';
...
export default function AssetForm() {
  ...
  const [formData, setFormData] = useState<AssetRequest & { propertyId?: string }>({
    name: '',
    type: '',
    location: '',
  ...
  useEffect(() => {
    propertiesApi.getAll()
      .then((res: any) => setProperties((res as any).data?.data || (res as any).data))
      .catch(() => {}); // fail silently
  }, []);
    ...
    <div className="max-w-xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">
              Property <span className="text-xs text-slate-400">(optional)</span>
            </label>
            <select
              name="propertyId"
